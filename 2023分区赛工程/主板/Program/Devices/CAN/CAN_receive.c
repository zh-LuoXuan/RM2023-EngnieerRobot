...
//返回upender电机变量地址，通过指针方式获取原始数据
const EncoderProcess_t* get_Upender_EncoderProcess_Point(uint8_t i)
{
    return &upenderMotor[i];
}
...
/**
  * @brief  CAN1发送矿石翻转机构控制命令
  * @param  PID输出值
  * @retval void
  */
void CAN1_CMD_UPENDER(int16_t data1 , int16_t data2)
{
	{
		CanTxMsg TxMessage;
		TxMessage.StdId = UPENDER_STDID;
		TxMessage.IDE = CAN_ID_STD;
		TxMessage.RTR = CAN_RTR_DATA;
		TxMessage.DLC = 0x08;
		TxMessage.Data[0] = data1>>8;
...
/**
  * @brief  CAN2发送矿石翻转机构控制命令
  * @param  PID输出值
  * @retval void
  */
void CAN2_CMD_UPENDER(int16_t data1 , int16_t data2)
{
	{
		CanTxMsg TxMessage;
		TxMessage.StdId = UPENDER_STDID;
...
/**
  ...
  * @retval void
  */
void CAN1_CMD_IO(int key_IO)
{
		CanTxMsg	TxMessage;

    TxMessage.StdId = CUSTOM_TOPMOVE_KEY_ID;	 
    TxMessage.IDE = CAN_ID_STD;
    TxMessage.RTR = CAN_RTR_DATA;		  
    TxMessage.DLC = 0x08;							 

    TxMessage.Data[0]=(u8)key_IO;
			...
			case UPENDER_M2_ID:
				{
					static uint8_t i = 0;
					//处理电机ID号
					i = RxMessage->StdId - UPENDER_M1_ID;
					//处理电机数据宏函数
					EncoderProcess(&upenderMotor[i], RxMessage);
					break;
				}
			case CUSTOM_VACUUM_CMD_ID:
				{
					CMD_vacuum = RxMessage->Data[0];
					if(CMD_vacuum == 1)
					{
						VACUUM_ON;
		        PRESSURE_OFF;
					}
					else if(CMD_vacuum == 0)
					{
						VACUUM_OFF;
						PRESSURE_ON;
						vTaskDelay(1000);
